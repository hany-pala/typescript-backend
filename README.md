...
## 1. Outline
### 1.1. Introduction
[![Build Status](https://github.com/inspiration33/typescript-backend/workflows/build/badge.svg)](https://github.com/inspiration33/typescript-backend/actions?query=workflow%3Abuild)
...
| Before          | After
|-----------------|----------------------------------------
| inspiration33 | Your account or corporation name
| typescript-backend      | Your own project name
| hane Han       | Author name
| test_db_schema  | Database schema to connnect
| https://github.com/inspiration33/typescript-backend | Your repository URL
...
When you're planning to deploy the backend server, read the [INFRASTRUCTURE.md](INFRASTRUCTURE.md) file and follow the steps. Following the steps, you may run some executable programs of the [src/executable](src/executable) directory.
  ...
  - [src/](src/): TypeScript Source directory
    - [src/api/](src/api/): Client SDK that would be published to the `@inspiration33/typescript-backend-api`
...
Also as you can see from the [package.json](package.json) file, this project requires the private npm module `@inspiration33`, provided from the Github. Therefore, to develop this backend server, you've configure the `.npmrc` file. Open the below link and complete the configuration.
...
### 3.2. SDK
[`@inspiration33/typescript-backend`](https://github.com/inspiration33/typescript-backend) provides SDK (Software Development Kit) for convenience.

For the client developers who are connecting to this backend server, [`@inspiration33/typescript-backend`](https://github.com/inspiration33/typescript-backend) provides not API documents like the Swagger, but provides the API interaction library, one of the typical SDK (Software Development Kit) for the convenience.
...
You know what? All of CI processes, like installing required programs into the clean Ubuntu and compiling and running the Test Automation Program, are defined in the [`.github/workflows/build.yml`](.github/workflows/build.yml) script file. Also <font color="green">passing</font> or <font color="red">failure</font> expressed from the above [badge](https://github.com/inspiration33/typescript-backend/actions?query=workflow%3Abuild) represents outcome of the CI test.
